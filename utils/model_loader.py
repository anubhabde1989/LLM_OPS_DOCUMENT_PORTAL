...
class ModelLoader:
    ...
    def load_llm(self):
        ...
        """
        """Load LLM dynamically based on provider in configuration"""
        llm_block = self.config["llm"]
        # Default Provider or choose from environment variable
        ...
        if provider == "google":
            ...
            llm = ChatGroq(model=model_name, api_key=self.api_keys["GROQ_API_KEY"], temperature = temperature, max_tokens = max_tokens) 
            return llm 
        elif provider == "openai":
            return ChatOpenAI(model=model_name, api_key=self.api_keys["OPENAI_API_KEY"], temperature = temperature, max_tokens = max_tokens)
        else:
...
if __name__ == "__main__":
    ...
    #Test LLM loading based on YAML configuration
    llm = loader.load_llm()
